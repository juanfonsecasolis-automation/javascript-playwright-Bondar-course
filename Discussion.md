...
## Conclusions
If the size of the team is large, and the manual and automation testing roles are isolated, then resources focused on automation will have enough time to obtain expertise in JS/TS/Playwright, which are have a complex syntax and steep learning curve for non-web developers. In practice, we didn't noticed the limitations of TS as a not-fully object-oriented programming language. Also, Playwright seems a good alternative to Selenium, as it adds an extra layer of abstraction that makes interaction with web components and assertions easier. If the size of team is small and resources need to jump from one role into another then Python/Playwright might be a better option.
